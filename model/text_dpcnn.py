...
# Author: Ai Xingsheng (felixai@tencent.com)
...
class TextDPCNNEstimator(tf.estimator.Estimator):
    def __init__(self, data_processor, model_params):
        ...
        def _convolution(inputs, num_filters, name):
            ...
            with tf.variable_scope("two_conv-%s" % name):
                ...
                W1 = tf.get_variable(name="W1-%s" % name, shape=filter_shape,
                ...
                W2 = tf.get_variable(name="W2-%s" % name, shape=filter_shape,
                                     initializer=initializer_normal)
        ...
        def _convolution_block(inputs, num_filters, name):
            ...
            with tf.variable_scope("pooling-%s" % name):
                pooled = tf.nn.max_pool(inputs, ksize=[1, 3, 1, 1],
        ...
        def _model_fn(features, labels, mode, params):
            ...
            with tf.variable_scope("dpcnn") as scope:
                ...
                W0 = tf.get_variable(name="W0", shape=filter_shape,
