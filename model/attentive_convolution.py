...
# References "Attentive Convolution"
# which can be seen at https://arxiv.org/abs/1710.00519
...
class AttentiveConvNetEstimator(tf.estimator.Estimator):
    ...
    def __init__(self, data_processor, model_params):
        ...
        def _model_fn(features, labels, mode, params):
            ...
            mat_hidden1 = tf.get_variable("mat_hidden1",
            ...
            mat_hidden2 = tf.get_variable("mat_hidden2",
            ...
            def _attentive_context(source, focus, name="context_generate"):
                ...
                mat_tx = tf.get_variable(name + "mat_tx", shape=[mat_dimension,
                    mat_dimension], initializer=tf.random_uniform_initializer(
                        -1.0 * pow(3.0 / dimension, 0.5),
                        pow(3.0 / dimension, 0.5)))
                mat_ta = tf.get_variable(name + "mat_ta", shape=[dimension,
                    mat_dimension], initializer=tf.random_uniform_initializer(
                        -1.0 * pow(3.0 / dimension, 0.5),
                        pow(3.0 / dimension, 0.5)))
