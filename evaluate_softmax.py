...
#       threshold =  config.eval.threshold
        ...
        if multi_label:
            unknown_labels = set(
                [y for x in set(test_classname) for y in x.split('/') if not y in dict_class_int2string.values()])
        else:
  ...
  #          top_k = 3
    ...
    def save(self):
        ...
        """
        if not self.confusion_matrix_list is None:
